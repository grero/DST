...
cathode = results.parameters.cathode;
numdatapoints = 3;
...
serialPort = results.parameters.serialport;
channel = 0;
pulseMag = results.parameters.pulsemag;
pulseDur = results.parameters.pulsedur;
numPulses = results.parameters.numpulses;
interDur = results.parameters.interdur;
stimprob = results.parameters.stimprob;
baudRate = results.parameters.baudrate;
...
errorCode = StimSerialController(serialPort, baudRate, 0, channel, pulseMag, pulseDur, numPulses, interDur);
    ...
    while continueTask == 1
        ...
        while startTask == 0
            ...
            if GetSecs - fixationIni > Stimulation_onset && stimulation == 0
                if rand < stimprob
                    if mouse == 0
                        Eyelink('Message', num2str([anode, cathode, channel, pulseMag, pulseDur, numPulses, interDur]));
                    end
                    errorCode = StimSerialController(serialPort, baudRate, 1, channel, pulseMag, pulseDur, numPulses, interDur);
                end
                stimulation = 1;
            end
...
%                     if mouse == 0
%                         Eyelink('Message', num2str([anode cathode]));
%                     end
...
return
=======
function myDST1_el(homeDir,results)
