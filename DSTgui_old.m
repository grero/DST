...
results.parameters.selected_locations = handles.metricdata.selected_locations;
...
handles.metricdata.selected_locations  = selected_locations;
...
% handles.metricdata.eyedatafile = 'load eye calibration file';
...
all_handles = [handles.fixation,handles.minfix,handles.minprestim,handles.maxprestim,handles.mintarget,handles.maxtarget,handles.minblank...
    ...
    ,handles.maxiti,handles.fixsize,handles.fixwin,handles.fixcol,handles.horgnum,handles.vergnum,handles.maxnumtarget,handles.mindistractor,handles.maxdistractor...
    ,handles.rewardprob,handles.rewardduration,handles.manualrewardduration,handles.blocknum,handles.maxcorrecttrials,handles.mouse,handles.respond_center...
    ,handles.targetcol,handles.distractorcol,handles.buffertime,handles.fixtime,handles.prestimcontrast,handles.stimcontrast,handles.responsecontrast];
...
function stimselect_Callback(hObject, eventdata, handles)
% hObject    handle to save (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% selected_locations = advanced_gui(handles.metricdata.horgnum,handles.metricdata.vergnum);
% handles.metricdata.selected_locations = {selected_locations};

selected_locations = handles.metricdata.selected_locations;
    ...
    for tempx = 1:vergnum
        if tempy == (horgnum+1)/2 && tempx == (vergnum+1)/2
            ...
            if isempty(selected_locations)
                ...
                selected = 0;
                for i = 1:size(selected_locations)
                    if selected_locations(i,1) == tempy && selected_locations(i,2) == tempx
                        bottom = height-10 - tempx*square_height;
                        hsurf{tempx,tempy} = uicontrol('Style','togglebutton',...
                            'Position',[left,bottom,square_width,square_height],...
...
switch str{val};
    case 'Set 1' % User selects Peaks.
        ...
        on_handles = [handles.fixation,handles.minfix,handles.feedback,handles.miniti,handles.maxiti,handles.fixsize,handles.rewardprob...
            ,handles.rewardduration,handles.manualrewardduration,handles.fixwin,handles.fixcol,handles.mouse];
    ...
    case 'Set 2' % User selects Peaks.
        ...
        on_handles = [handles.fixation,handles.minfix,handles.minprestim,handles.maxprestim,handles.response,handles.minresponse...
            ...
            ,handles.vergnum,handles.targetcol,handles.rewardprob,handles.rewardduration,handles.manualrewardduration,handles.blocknum,handles.fixcol,handles.buffertime];
     ...
     case 'Set 3' % User selects Membrane.
        ...
        on_handles = [handles.fixation,handles.minfix,handles.minprestim,handles.maxprestim,handles.mintarget,handles.maxtarget,handles.minblank...
            ...
            ,handles.buffertime,handles.fixtime];
...
handles.metricdata.selected_locations = 0;
guidata(hObject,handles)
plot_stimulus(handles)
...
if handles.metricdata.selected_locations ~= 0
    ...
    for x = 1:size(sstim,1)
        % fill([sstim(x,1)-1,sstim(x,1)-1,sstim(x,1),sstim(x,1)],[sstim(x,2)-1 sstim(x,2) sstim(x,2) sstim(x,2)-1],'r')
